<template>
  <div>
    ...
    <el-card>
      ...
      <!-- 步骤条区域 -->
      <!-- :active="activeIndex - 0" 作用是将'0'转换为0 -->
...
export default {
  data() {
    return {
      ...
      //图片上传组件的headers请求头对象
      headerObj: {
        Authorization: window.window.sessionStorage.getItem("token")
      },
      previewPath: "",
  ...
  methods: {
    ...
    handleRemove(file) {
      ...
      // 2.从pics 数组中，找到这个图片对应的索弓|值
      const i = this.addForm.pics.findIndex(x => x.pic === filePath);
      // 3.调用数组的splice方法，把图片信息对象，从pics数组中移除I
    ...
    add() {
      this.$refs.addFormRef.validate(async valid => {
        ...
        //执行添加的业务逻辑
        //避免和基本信息中的级联选择器产生冲突 lodash cloneDeep(obj) 深拷贝
        const form = _.cloneDeep(this.addForm)
        form.goods_cat = form.goods_cat.join(',')
        //处理动态参数(join 是为了将数组变为字符串)
